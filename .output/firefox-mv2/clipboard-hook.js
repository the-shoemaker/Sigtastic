(() => {
  ...
  const getDirectTaskTypeValues = (taskType) => {
    const normalized = normalizeTaskType(taskType);
    if (!normalized || !TASK_TYPE_VALUES[normalized]) {
      return [];
    }

    return TASK_TYPE_VALUES[normalized];
  };

  const applyTaskTypeValuesToShapes = (shapes, taskType) => {
    const targetValues = getDirectTaskTypeValues(taskType);
  ...
  const rememberTaskTypeForShapes = (shapes, taskType) => {
    for (const shape of shapes) {
      rememberTaskType(getObjectId(shape), taskType);
    }
  };
  ...
  const refreshEditor = (facade, selectedShapes) => {
    ...
    const selection = resolvedFacade ? getFacadeSelection(resolvedFacade) : [];
    ...
    for (const shape of selectedShapes) {
      ...
      try {
        if (typeof shape.layout === "function") {
          shape.layout();
        }
      } catch {}

      try {
        if (typeof shape._update === "function") {
          shape._update();
        }
      } catch {}
    }
    ...
    try {
      if (resolvedFacade && typeof resolvedFacade.setSelection === "function" && selection.length > 0) {
        resolvedFacade.setSelection([]);
        resolvedFacade.setSelection(selection);
      }
    } catch {}
  ...
  const settleTaskTypeVisuals = async (taskType, facade, selectedShapes) => {
    const canSyncThroughTaskTypeUi = Boolean(
      findTaskTypePropertyContext() ||
      findTaskTypeExtComponent() ||
      findTaskTypeRowElements(),
    );

    refreshEditor(facade, selectedShapes);
    await delay(30);

    if (canSyncThroughTaskTypeUi) {
      try {
        await applyTaskTypeActionDom(taskType);
      } catch {}
    }
  ...
  const querySelectionInfo = () => {
    ...
    let taskType = null;
    const domTaskType = isTask ? readTaskTypeFromDom() : null;
  ...
  const applyTaskTypeAction = async (taskType, preferredShapeId) => {
    ...
    if (taskShapes.length > 0) {
      const changed = applyTaskTypeValuesToShapes(taskShapes, normalizedType);

      if (changed) {
        rememberTaskTypeForShapes(taskShapes, normalizedType);
        await settleTaskTypeVisuals(normalizedType, facade, selectedShapes);
        return { ok: true };
      }
    ...
    const domResult = await applyTaskTypeActionDom(normalizedType);
    if (domResult.ok) {
      const confirmedTaskType = readTaskTypeFromDom() || normalizedType;
      if (taskShapes.length > 0) {
        applyTaskTypeValuesToShapes(taskShapes, confirmedTaskType);
      }
      rememberTaskTypeForShapes(taskShapes, confirmedTaskType);
      await settleTaskTypeVisuals(confirmedTaskType, facade, selectedShapes);
    }
